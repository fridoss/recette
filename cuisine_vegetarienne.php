...
try {
    $requete_recettes = $connexion->prepare("SELECT * FROM recettes WHERE type_cuisine = 'végétarienne'");
    $requete_recettes->execute();
    $recettes_vegetariennes = $requete_recettes->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "Erreur lors de la récupération des recettes végétariennes : " . $e->getMessage();
    $recettes_vegetariennes = [];
}


$commentaires_par_recette = [];
foreach ($recettes_vegetariennes as $recette) {
    try {
...
<head>
    ...
    <title>Passionné de Cuisine - Cuisine Végétarienne</title>

    <style>
        body {
            background-image: url('imgveg.jpeg'); 
...
<body>
    <div class="container">
        ...
        <h2>Découvrez nos Délicieuses Recettes Végétariennes</h2>

        <select id="selection-recette" onchange="afficherRecette(this.value)">

            <?php foreach ($recettes_vegetariennes as $recette): ?>
                        ...
                        <div class="commentaires-section">
                            ...
                            <div class="commentaire-form">
                                ...
                                <form method="post" action="">
									...
									<input type="hidden" name="type_cuisine" value="vegetarienne">
