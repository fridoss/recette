...
try {
    $requete_recettes = $connexion->prepare("SELECT * FROM recettes WHERE type_cuisine = 'salée'");
    $requete_recettes->execute();
    $recettes_salees = $requete_recettes->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "Erreur lors de la récupération des recettes salées : " . $e->getMessage();
    $recettes_salees = [];
}


$commentaires_par_recette = [];
foreach ($recettes_salees as $recette) {
    try {
...
<head>
    ...
    <title>Passionné de Cuisine - Cuisine Salée</title>
...
<body>
    <div class="container">
        ...
        <h2>Découvrez nos Délicieuses Recettes Salées</h2>

        <select id="selection-recette" onchange="afficherRecette(this.value)">

            <?php foreach ($recettes_salees as $recette): ?>
                        ...
                        <div class="commentaires-section">
                            ...
                            <div class="commentaire-form">
                                ...
                                <form method="post" action="">
									...
									<input type="hidden" name="type_cuisine" value="salée">
